...
export class ContactService {
    ...
    public setFilter(contactFilter:ContactFilter){
        this._contactsFilter$.next({...contactFilter})
            ...
            .pipe(
                tap(contacts => {
                    ...
                    if (filterBy ) {
                        ...
                        console.log('contacts after filter',contacts);
    ...
    public getContactById(id: any): Observable<Contact> {
        const contact = from(storageService.get(ENTITY, id))
                return contact
    ...
    private _filter(contacts: Contact[], contactFilter:ContactFilter) {

        return contacts.filter(contact => {
            return contact.name.toLocaleLowerCase().includes(contactFilter.name) &&
                contact.phone.toLocaleLowerCase().includes(contactFilter.phone) &&
                contact.email.toLocaleLowerCase().includes(contactFilter.email)
        })
    }
    ...
    private _createContacts() {
        ...
        contacts.forEach(contact=>_setNextPrevContactId(contact))
       function  _setNextPrevContactId(contact:any) {
    
            const contactIdx = contacts.findIndex((currContact) => currContact._id === contact._id)
            contact.nextContactId = contacts[contactIdx + 1] ? contacts[contactIdx + 1]._id : contacts[0]._id
